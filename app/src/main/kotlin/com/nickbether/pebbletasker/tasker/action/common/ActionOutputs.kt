 ...
 *
 * Every action's output object carries the result block (%pbl_ok / %pbl_err / %pbl_errmsg / %pbl_json)
 ...
 *     the typed output, using [jsonBlob] for %pbl_json and [CommandResult.data] lookups for scalars.
 */
object ActionOutputs {

    /**
     * Build the canonical %pbl_json blob for an action result. Includes ok + (on failure) err/errmsg,
     * and every key in the bridge data map. Tasker 5.12+ reads `%pbl_json.<key>`.
     */
    ...
    /** "true"/"false" string for an [PebbleActionRunner.CommandResult.ok], for the %pbl_ok scalar. */
    fun okStr(result: PebbleActionRunner.CommandResult): String = result.ok.toString()

    /** %pbl_err scalar: empty when ok, else the int code as a string. */
